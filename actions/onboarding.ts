...
async function mergeWithExistingAnswers(
...
): Promise<Partial<OnboardingFormValues>> {
  ...
  const existing =
    data?.answers && typeof data.answers === "object"
      ? (data.answers as Partial<OnboardingFormValues>)
      : {};
...
async function persistAnswers(
  ...
  if (existing) {
    const payload: { answers: Partial<OnboardingFormValues>; completed_at?: string | null } = {
      answers,
    };
